...
impl Parser {
    ...
    pub fn call_expr(&mut self) -> Expression {
        ...
        while let Some(param_token) = self.tokens.get(self.cursor) {
            match param_token.token_type {
                ...
                TokenType::Identifier => {
                    let literal = self.parse_literal(param_token.clone(), Some(self.cursor), false);
                    self.cursor += 1;
    ...
    pub fn call_expr_as_var(&mut self) -> Expression {
        ...
        while let Some(param_token) = self.tokens.get(self.cursor) {
            match param_token.token_type {
                ...
                TokenType::Identifier => {
                    let literal = self.parse_literal(param_token.clone(), Some(self.cursor), true);
                    self.cursor += 1;
    ...
    fn parse_literal(&mut self, token: Token, cursor: Option<usize>, is_call: bool) -> Expression {
